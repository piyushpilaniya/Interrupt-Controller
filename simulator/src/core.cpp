...
void Core::decode() {
	...
	of_ex.isMovz.Write(temp_isMovz);
	of_ex.isRetz.Write(temp_isRetz);
...
void Core::execute() {
	...
	bool temp_isMovz = of_ex.isMovz.Read();
	bool temp_isRetz = of_ex.isRetz.Read();
	...
	if (temp_isMovz){
		//pprint(2)<<"MOVZ operation"<<endl;
		temp_aluResult = temp_B;
	}

	if (temp_isRetz){
		//pprint(2)<<"RETZ operation"<<endl;
		temp_aluResult = temp_A;
	}
	...
	ex_ma.isMovz.Write(temp_isMovz);
	ex_ma.isRetz.Write(temp_isRetz);
...
void Core::mem_access() {
	...
	bool temp_isMovz = ex_ma.isMovz.Read();
	bool temp_isRetz = ex_ma.isRetz.Read();
	...
	ma_rw.isMovz.Write(temp_isMovz);
	ma_rw.isRetz.Write(temp_isRetz);
...
void Core::write_back() {
	...
	bool temp_isMovz = ma_rw.isMovz.Read();
	bool temp_isRetz = ma_rw.isRetz.Read();
